...
export default function PaymentSuccess({
  ...
  searchParams: { amount: number; subtotal: number; shipping: number };
}) {
  const { cartItems, clearCart } = useShoppingCart();
        ...
        <div className="mt-4 gap-4 flex flex-col items-center">
          {cartItems.map((cartItem) => {
            ...
            return (
              <div key={cartItem.id}>
                <div className="flex justify-between w-full md:w-[700px] px-2">
                  <div className="flex flex-row items-start gap-4 w-full md:w-[700px]">
                    <Image
                      src={matchingAsset.image}
                      width={300}
                      height={300}
                ...
                <hr className="border-t-2 border-sol-green w-full md:w-[700px] mx-auto mt-2" />
              </div>
            );
          })}
        </div>
