...
interface PurchaseModalProps {
  ...
  transactionSignature: string;
  shippingCost: number;
  subtotalUSD: number; // Add this prop for USD subtotal
  subtotalSOL: string; // Add this prop for SOL subtotal (as a string to maintain precision)
}
...
const PurchaseModal: React.FC<PurchaseModalProps> = ({
  ...
  transactionSignature,
  shippingCost,
  subtotalUSD, // Destructure USD subtotal
  subtotalSOL, // Destructure SOL subtotal
}) => {
  const { clearCart } = useShoppingCart();
